...
def get_number(setting: str) -> int:
    ...
    """
    while True:
        input_value = input(CHANGE_SETTING_PROMPT.format(setting))
        if input_value.isnumeric():
            return int(input_value)
        else:
            print(ENTRY_NOT_NUMBER)
...
def get_guess(min_val: int, max_val: int) -> int:
    ...
    """
    while True:
        input_value = input(ENTRY_PROMPT.format(min_val, max_val))
        if input_value.isnumeric():
            guess = int(input_value)
            if min_val <= guess <= max_val:
                return guess
            else:
                print(ENTRY_OUT_OF_RANGE.format(min_val, max_val))
        else:
            print(ENTRY_NOT_NUMBER)
...
def get_command() -> int:
    ...
    """
    while True:
        input_command = input(ACTION_PROMPT).strip().casefold()
        if input_command == COMMAND_TXT_PLAY:
            return OPTION_PLAY
        elif input_command == COMMAND_TXT_QUIT:
            return OPTION_QUIT
        elif input_command == COMMAND_TXT_SET_NUM:
            return OPTION_SET_NUM
        elif input_command == COMMAND_TXT_SET_HIGH:
            return OPTION_SET_HIGH
        elif input_command == COMMAND_TXT_SET_LOW:
            return OPTION_SET_LOW
        elif input_command == COMMAND_TXT_RESET:
            return OPTION_RESET
        elif input_command == COMMAND_TXT_HELP:
            print(HELP_PROMPT)
        else:
...
def human_play(number: int, max_attempts: int, min_val: int, max_val: int) -> int:
    ...
    print(FEEDBACK_ATTEMPTS_NONE.format(PLAYER1, number, max_attempts))
    return -1
...
def computer_play(number: int, max_attempts: int, min_val: int, max_val: int) -> int:
    ...
    while attempts < max_attempts:
        ...
        result = check(guess, number)
        if result == CHECK_CORRECT:
            print(FEEDBACK_ATTEMPTS.format(PLAYER2, attempts, number))
            return attempts
